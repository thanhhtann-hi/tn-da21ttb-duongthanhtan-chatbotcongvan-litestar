/********************************************************************
 * File  : src/modules/admin/static/js/admin_notify_edit_modal.js
 * Updated: 2025-08-12 (v2.7 – ES5-safe)
 * Lý do : Giữ đủ filter (v/sort/q/start/end/page/per_page) khi cập nhật,
 *         chặn double-submit, chỉ swap #notify-list-region, toast 1 lần,
 *         phục hồi khi lỗi. Bổ sung htmx:configRequest & QS-fallback an toàn.
 ********************************************************************/

/* Focus textarea khi modal mở */
function focusNotifyEditModalTextarea() {
    setTimeout(function () {
        var modal = document.getElementById('notify-edit-modal');
        if (!modal) return;
...
function closeNotifyEditModal() {
    var ov = document.getElementById('notify-edit-modal-overlay');
    if (ov && ov.remove) ov.remove();
    var md = document.getElementById('notify-edit-modal');
    if (md && md.remove) md.remove();
}

/* Bind nút đóng, overlay, ESC và click ra ngoài modal */
function bindNotifyEditModalClose() {
    document.body.addEventListener('click', function (e) {
        if (!e || !e.target) return;

        if (e.target.id === 'notify-edit-modal-overlay') { closeNotifyEditModal(); return; }
        if (e.target.id === 'notify-edit-modal-close') { closeNotifyEditModal(); return; }
        if (e.target.closest) {
            var x = e.target.closest('#notify-edit-modal-close');
            if (x) { closeNotifyEditModal(); return; }
        }
        if (e.target.id === 'notify-edit-modal') { closeNotifyEditModal(); return; }
    });

    document.body.addEventListener('keydown', function (e) {
        if ((e.key === 'Escape' || e.keyCode === 27) && document.getElementById('notify-edit-modal')) {
            closeNotifyEditModal();
        }
    });
}
...
/* Lấy filter hiện tại (ưu tiên helper mới, fallback DOM) */
...
/* === Giữ đủ filter: v/sort/q/start_date/end_date/page/per_page === */
function ensureEditFormKeepsFilters(form) {
    if (!form || form.dataset.filterBound === '1') return;

    try { form.removeAttribute('hx-vals'); } catch (e) { }

    /* Inject trước khi submit (và chặn double-submit) */
    form.addEventListener('submit', function () {
        if (form.dataset._submitting === '1') return;
        form.dataset._submitting = '1';

        window.__notifyJustEdited = true;

        var btn = form.querySelector('#notify-edit-submit-btn');
        if (btn) btn.disabled = true;

        var f = _getCurrentFilters();
        var hasRange = !!(f.start_date && f.end_date);

        // bơm hidden để backend _extract_filters đọc được từ form
        ...
        // merge QS vào hx-put (phòng trường hợp backend ưu tiên query)
        var base = form.getAttribute('hx-put') || form.action || ("/admin/notify/" + (form.getAttribute('data-id') || ''));
    ...
    form.addEventListener('htmx:configRequest', function (e) {
        try {
            ...
            // vá path với QS helper (tránh mất tham số khi server đọc query)
            var qs = (typeof window.__notifyFilterQS === 'function') ? (window.__notifyFilterQS() || '') : '';
...
/* Validate form: disable cập nhật nếu thiếu/không đổi */
function attachNotifyEditFormLogic() {
    var form = document.querySelector('#admin-notify-edit-form');
    var submitBtn = document.querySelector('#notify-edit-submit-btn');
    ...
    var visibleEl = form.querySelector('input[name="notify_visible"]');

    function currentSnapshot() {
        var txt = textarea ? (textarea.value.trim()) : '';
        var roles = '';
        for (var i = 0; i < checkboxes.length; i++) {
            if (checkboxes[i].checked) roles += (roles ? ',' : '') + checkboxes[i].value;
        }
        var vis = (visibleEl && visibleEl.checked) ? '1' : '0';
        return txt + '|' + roles + '|' + vis;
    }

    var initialValue = currentSnapshot();
    ...
    function toggleSubmit() {
        ...
        var changed = currentSnapshot() !== initialValue;
        submitBtn.disabled = !(anyChecked && hasContent && changed);
    }
    ...
    if (!form.dataset.boundRoles) {
        ...
        if (visibleEl) visibleEl.addEventListener('change', toggleSubmit);
        form.dataset.boundRoles = '1';
    }

    ensureEditFormKeepsFilters(form);
    toggleSubmit();
}

/* Sau khi modal được load động, bind lại các sự kiện */
function rebindAdminNotifyEditModalEvents() {
    focusNotifyEditModalTextarea();
    attachNotifyEditFormLogic();
}

/* Gắn các handler này 1 lần duy nhất khi SPA load */
bindNotifyEditModalClose();
...
document.body.addEventListener('htmx:afterOnLoad', function (evt) {
    ...
    if (tid === 'admin-notify-modal-root' || tid === 'notify-edit-modal') {
        rebindAdminNotifyEditModalEvents();
    }
});

/* Khi list được swap (cập nhật xong) → đóng modal + toast */
document.body.addEventListener('htmx:afterSwap', function (evt) {
    var t = evt && evt.detail ? evt.detail.target : null;
    if (!t) return;
    if (t.id === 'notify-list-region') {
        closeNotifyEditModal();
        if (window.__notifyJustEdited) {
            if (window.Toast && window.Toast.show) window.Toast.show('Cập nhật thông báo thành công!', 'success', 3000);
            window.__notifyJustEdited = false;
        }
        var form = document.querySelector('#admin-notify-edit-form');
...
/* Nếu lỗi response/swap/send → bật lại nút submit, reset cờ & báo lỗi */
function _reEnableEditOnError(elt) {
    var form = (elt && elt.id === 'admin-notify-edit-form') ? elt : document.querySelector('#admin-notify-edit-form');
    if (!form) return;
    form.dataset._submitting = '';
    var btn = form.querySelector('#notify-edit-submit-btn');
    if (btn) btn.disabled = false;
    if (window.Toast && window.Toast.show) window.Toast.show('Cập nhật thất bại. Vui lòng thử lại!', 'error', 3000);
}

document.body.addEventListener('htmx:responseError', function (evt) {
    var d = (evt && evt.detail) ? evt.detail : {};
    _reEnableEditOnError(d.elt ? d.elt : null);
});
...
/* Nếu modal mở sẵn (không qua HTMX) -> bind ngay */
if (document.getElementById('notify-edit-modal')) {
    rebindAdminNotifyEditModalEvents();
}
